...
const replacement = `<div class="rrs-track">
          <!-- SLIDE 1 -->
          <div class="rrs-slide">
...
const processFile = (file) => {
  ...
  // Replace the contents of <div class="rrs-track"> ... </div> with our replacement
  // We will find the start of rrs-track and the end
  const startIndex = content.indexOf('<div class="rrs-track">');
  if (startIndex !== -1) {
    // Find the closing tag of this div. Since it contains many nested divs,
    // let's just find the closing tag before <!-- Navigation Buttons -->
    // which immediately follows the track
    const endIndex = content.indexOf('<!-- Navigation Buttons -->');
    if (endIndex !== -1) {
      // The closing </div> is right before '<!-- Navigation Buttons -->'
      // We will replace from startIndex to endIndex (exclusive, we want to keep <!-- Navigation Buttons -->)
      
      const before = content.substring(0, startIndex);
      const after = content.substring(endIndex);
      
      // Wait, there's a </div> closing the <div class="rrs-track"> right before <!-- Navigation Buttons -->
      // So if I replace exactly up to `<div class="rrs-nav">` or `<!-- Navigation Buttons -->`,
      // I just need to make sure my replacement has `<div class="rrs-track">` ... `</div>`
      // My replacement ends with `</div>` (for the track)
      // BUT let's just replace the whole section.
      const closingDivMatch = content.substring(0, endIndex).lastIndexOf('</div>');
