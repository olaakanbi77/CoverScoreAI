...
class QuestionPackFactory {
  ...
  // 1. Conversation Engine (Welcome, Tell Me More)
  buildConversationEngine(config) {
    const p = config.prefix;
    const indStr = config.audience;
    ...
    return [
      ...
      },
      {
        ...
        industry: config.ind,
        pillar: 'General',
        question: `CoverScore helps ${type === 'business' ? 'business owners' : 'individuals'} identify hidden risks before they become costly incidents.\n\nWould you like to continue?`,
        question_type: 'single_choice',
  ...
  buildLeadCaptureEngine(config) {
    ...
    if (config.type === 'business') {
      qs = [
        {
          id: `${p}_003`, industry: config.ind, pillar: 'General',
  ...
  buildQuestionPack(config) {
    ...
    let nextId = config.type === 'business' ? 7 : 5;
